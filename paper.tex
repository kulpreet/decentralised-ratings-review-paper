...
\title{Chlu \\ Reviews and Ratings Verified by Payments on a Blockchain}
\date{}
\author{
Kulpreet Singh
\and 
...
too. However, these marketplaces keep the user's reputations locked to
...
user's reputations can be saved on publicly accessible systems while
...
reveal his reputation only to a select customer or on a
...
Chlu is a decentralised platform for making payments and saving vendor
reputations. Chlu enables vendors to build their reputation on a
decentralised blockchain so they are not confined to a walled garden
...
as providing privacy to the vendor in case he wants to disable any
reviews and ratings for his services.

This feature also allows a vendor to leave a marketplace and thus stop
sharing his reputation with the marketplace, even though the
...
received. Once a vendor shares his reputation with a marketplace all
...
Marketplaces are free to compute vendor's reputation scores based on
...
\begin{figure}
\centering
\includegraphics[width=0.5\textwidth]{../flexcoin-diagrams/reputation.eps}
...
$\texttt{Proof of Payment Request}, \texttt{PoPR} = (I, In, C, T, A, M, Sig(S_v, I,In,C,
T, A, M))$, where $S_v$ is the vendor's signing key the vendor has
shared with the marketplace and Sig is the operation of concatenating
all the arguments apart from $S_v$, hashing it and and then signing
the hash with $S_v$
...
\item[$RR$, Review Record] An IPFS content address where the review is
  stored
\end{description}
...
\item[$S_{ve}$ and $P_{ve}$ is the vendor's encryption key pair] The
  ...
  vendor to control who can view his review details. To share the
...
\begin{figure}
\centering
\includegraphics[width=0.6\textwidth]{../flexcoin-diagrams/vendor-wallet-init.eps}
...
\item[RR] The \texttt{Review Record} includes the review and rating
  ...
  plaintext, and the vendor has to include this hash whenever he
  publishes the review to a marketplace. This allows anyone to verify
...
\begin{figure}
\centering
\includegraphics[width=0.6\textwidth]{../flexcoin-diagrams/payment-processing.eps}
...
\begin{figure}
\centering
\includegraphics[width=0.6\textwidth]{../flexcoin-diagrams/validating-new-tx.eps}
\caption{\label{fig:validating-new-tx}Key distribution: Vendor}
\end{figure}
...
The above process describes how the vendor can share their review to
the marketplace using their wallet. It should be noted, however, that
this functionality can be provided by any service that watches the
blockchain for transactions to the vendor address and shares the
...
use this history to show top ratings on his own website. However, any
