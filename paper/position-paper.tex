...
\item[$Ptr$, Hash Pointer] This is a hash pointer to the most recent
  review written by the customer. See
...
developed by IPFS team is a promising avenue.
...
To address this potential problem we sign the review record with a
private key and includes a public key to verify the signature in the
review record. When the customer publishes an update to the review,
the customer includes a signature using the same private key, so that
the public key published in the original review record can be used to
verify that the update to the review record is written by the entity
...
when a payment was made and when a review is being written. The goal
of providing these timestamps is not provide a precise timestamp but
...
created response to a payment made in the past. The time limit beyond
which a marketplace wants to disregard reviews is set by individual
marketplaces. Chlu suggest this limit be set to $2$ months, but each
marketplace can set this limit independently.
...
\texttt{Ptr} in~\ref{sec:review-record-stored}. As shown in
